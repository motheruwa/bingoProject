...
const User = () => {
  ...
  const handleCreateOrUpdate = async () => {
    try {
      // eslint-disable-next-line
        const { data, error } = await (user.userName === 'addis3' ? supabase2 : user.userName === 'abebe'|| user.userName === 'abebe1' ? supabase3 : user.userName === 'dagi bingo' || user.userName === 'dani bingo' ? supabase5 : null)
            .from('algorithm')
